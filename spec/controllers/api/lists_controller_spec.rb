  ...
  describe "GET #show_users_lists" do
    context "when valid" do
      before do
        ...
        get :show_users_lists, format: :json, params: { id: @user.id }
      ...
      it "JSON body response contains expected lists attributes" do
        expect(@json_response.first.keys).to match_array(["created_at", "description", "id", "movies", "name", "public", "updated_at", "user_id"])
      end
      ...
      it "JSON body response contains expected list description" do
        expect(@json_response.first['movies']).to eq([])
      end
    ...
    context "when invalid" do
      ...
      context "when the user does not exist" do
        before do
          request.headers["AUTHORIZATION"] = "Bearer #{@token}"
          get :show_users_lists, format: :json, params: { id: "False id" }
          @json_response = JSON.parse(response.body)
        end
    ...
    context "when the user's token does not match the user to display" do
      before do
        request.headers["AUTHORIZATION"] = "Bearer #{@token}"
        get :show_users_lists, format: :json, params: { id: @user1.id }
        @json_response = JSON.parse(response.body)
      end
      it "returns http unauthorized" do
        expect(response).to have_http_status(:unauthorized)
